...
struct ReadSchedulingCheck: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "read-scheduling-check",
        abstract: "Benchmark experimental exact expert read scheduling on bounded original regions")
    ...
    func run() throws {
        ...
        do {
            let report = try Diagnostics.readSchedulingComponent(modelDir: ModelLocator.resolve(model))
            ...
            guard report.check.passed else { throw ValidationError("read scheduling checks failed") }
