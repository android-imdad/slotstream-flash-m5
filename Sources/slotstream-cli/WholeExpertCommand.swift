...
struct WholeExpertCheck: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "whole-expert-check",
        abstract: "Compare whole-expert storage with original reads on bounded JANG regions")
    ...
    func run() throws {
        ...
        do {
            let report = try Diagnostics.wholeExpertComponent(modelDir: ModelLocator.resolve(model), artifact: destination.appendingPathComponent("sample-layout"))
            ...
            guard report.check.passed else { throw ValidationError("whole-expert checks failed") }
