...
struct PrefetchCostsCheck: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "prefetch-costs-check",
        abstract: "Measure bounded original-reader costs for causal prefetch screening")
    ...
    func run() throws {
        ...
        do {
            let report = try Diagnostics.prefetchReaderCosts(modelDir: ModelLocator.resolve(model))
            ...
            guard report.check.passed else { throw ValidationError("prefetch cost checks failed") }
