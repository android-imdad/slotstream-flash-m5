...
SPEC = importlib.util.spec_from_file_location('prefill_compare', Path(__file__).with_name('compare.py'))
...
def fixture(root, chunk):
    ...
    report = {'format': 'slotstream-prefill-capture-v1', 'schema_version': 1,
              ...
              'plan': {'target_gb': 14, 'max_context_tokens': 4096, 'prefill_chunk': chunk,
                       'pool_slots': 2000 - chunk, 'mtp': False, 'vision': False, 'source': '--memory-gb',
                       'runtime_prefill_override': chunk, 'checkpoint_format': 'jang6S',
                       'runtime_prefix_cache_enabled': False, 'prefix_cache_max_tokens': 0},
              'snapshots': {}, 'routes': {}, 'route_events': []}
    report['plan']['memory_ledger'] = dict.fromkeys(C.LEDGER_KEYS, 0)
    ledger = report['plan']['memory_ledger']
    ledger.update(version=1, fixed_bytes=8_859_096_600, active_capacity_bytes=113_246_208,
                  prefill_bytes=chunk * 1_300_000, planning_margin_bytes=1_000_000_000)
    slots = (14_000_000_000 - ledger['fixed_bytes'] - ledger['prefill_bytes'] - 1_000_000_000) // C.JANG_RECORD_BYTES
    report['plan']['pool_slots'] = report['effective_pool_slots'] = slots
    ledger['pool_bytes'] = slots * C.JANG_RECORD_BYTES
    ledger['expected_peak_bytes'] = ledger['fixed_bytes'] + ledger['pool_bytes'] + ledger['prefill_bytes']
    passes = [chunk] * (len(prompt) // chunk) + ([len(prompt) % chunk] if len(prompt) % chunk else [])
    report['stats'] = {'prefillPasses': passes, 'prefillTokens': len(prompt), 'promptTokens': len(prompt)}
...
class ComparatorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dirs = [self.root / str(x) for x in (256, 384, 512)]

    def tearDown(self):
        self.temp.cleanup()

    def mutate(self, change, chunk=512):
        root = self.root / str(chunk)
        report = json.loads((root / 'report.json').read_text())
        change(report, root)
        seal(root, report)

    def array_change(self, field_path, transform, chunk=512):
    ...
    def test_empirical_control_scales_band(self):
        self.array_change(('snapshots', 'step8', 'tensors', 'ssm.0'), lambda x: x + np.float32(.1), 384)
        self.array_change(('snapshots', 'step8', 'tensors', 'ssm.0'), lambda x: x + np.float32(.2), 512)
        result = C.compare(self.dirs)
        self.assertTrue(result['passed'], result['errors'])
    ...
    def test_declared_384_requires_actual_control_plan_and_schedule(self):
        result = C.compare(self.dirs)
        self.assertTrue(result['passed'], result['errors'])
        self.assertEqual((result['reference_chunk'], result['empirical_control_chunk'], result['candidate_chunk']), (256, 384, 512))
        control = json.loads((self.dirs[1] / 'report.json').read_text())
        self.assertEqual(control['plan']['memory_ledger']['prefill_bytes'], 384 * 1_300_000)
        self.assertEqual(control['effective_pool_slots'], 846)
        self.assertEqual(control['stats']['prefillPasses'], [384, 384, 332])

    def test_512_artifacts_relabeled_as_384_rejected(self):
        report = json.loads((self.dirs[2] / 'report.json').read_text())
        report['effective_prefill_chunk'] = report['plan']['prefill_chunk'] = 384
        report['plan']['runtime_prefill_override'] = 384
        report['runtime_environment']['SLOTSTREAM_PREFILL_CHUNK'] = '384'
        seal(self.dirs[1], report)
        self.rejected('prefill workspace does not charge actual chunk')

    def test_relabelled_schedule_alone_rejected(self):
        self.mutate(lambda report, _: report['stats'].update(prefillPasses=[512, 512, 76]), 384)
        self.rejected('scheduled prefill passes differ')

    def test_wrong_pool_pricing_rejected(self):
        def change(report, _):
            report['effective_pool_slots'] -= 1
            report['plan']['pool_slots'] -= 1
        self.mutate(change, 384)
        self.rejected('pool bytes do not match')
    ...
    def test_hidden_memory_ledger_policy_drift_rejected(self):
        ...
        self.rejected('unexpected JANG planning margin')
    ...
    def test_control_greedy_drift_is_diagnostic_not_candidate_gate(self):
        ...
        for stage in C.STAGES:
            self.array_change(('snapshots', stage, 'tensors', 'logits'), change, 384)
        def fix_metadata(report, _):
            for snap in report['snapshots'].values():
                snap['greedy_token'] = 101
            report['normal_output_ids'] = [101]
        self.mutate(fix_metadata, 384)
        result = C.compare(self.dirs)
        self.assertTrue(result['passed'], result['errors'])
