...
384 is an empirical rechunk control, not an independently qualified configuration.
State and logit gates remain individual. Routing gates aggregate each phase across all
48 layers, matching Diagnostics+PrefillQualification.disagreement; layer metrics stay
visible as nongating diagnostics.
"""
...
ARM_CHUNKS = (256, 384, 512)
JANG_RECORD_BYTES = 3 * (640 * 2560 * 6 // 8 + 640 * 2560 // 64 * 8)
...
def optimizations_valid(opts):
    # Frozen Plan 020 host: Mac17,6 / Apple M5 Max / macOS build 25F80.
    # Optimizations.swift deploymentCandidate gates integrationCandidate.fusedRoPE
    # through OptimizationPlatform.qualifiedPartialRotation. That predicate only
    # admits native ARM64 Mac17,9 / Apple M5 Pro / 25G83, so deployed fusedRoPE is
    # FALSE here. The exact 54-field vector matches all saved Plan 018 run stats
    # and the native Plan 019 captures, which independently assert equality with
    # InferenceOptimizations.environment([:]). Do not accept either value loosely.
    ...
    on = {'compactStateWindows', 'compactMTPRow', 'compactNgramRows', 'automaticReadScope',
          ...
          'boundedOutputQueue', 'responsiveGovernor', 'completePromptCheckpoint', 'sharedRoPE'}
...
def load_arm(root, chunk, errors):
    ...
    try:
        ...
        require(512 < len(prompt) <= 4088, 'prompt does not exercise all chunks or exceeds context')
        ...
        require(plan.get('runtime_prefill_override') == chunk and plan.get('checkpoint_format') == 'jang6S',
                'runtime override/checkpoint format mismatch')
        require(plan.get('runtime_prefix_cache_enabled') is False and plan.get('prefix_cache_max_tokens') == 0,
                'plan retains prefix cache')
        require(ledger['prefill_bytes'] == chunk * 1_300_000, 'prefill workspace does not charge actual chunk')
        require(ledger['planning_margin_bytes'] == 1_000_000_000, 'unexpected JANG planning margin')
        peak_fields = ('fixed_bytes', 'pool_bytes', 'additional_active_bytes', 'retained_capacity_bytes',
                       'retained_recurrent_bytes', 'prefill_bytes', 'long_context_reserve_bytes',
                       'mtp_resident_bytes', 'vision_resident_bytes', 'diagnostic_reserved_bytes')
        require(ledger['expected_peak_bytes'] == sum(ledger.get(k, 0) for k in peak_fields), 'memory ledger sum mismatch')
        require(ledger['pool_bytes'] == report['effective_pool_slots'] * JANG_RECORD_BYTES,
                'pool bytes do not match effective JANG_6S slots')
        available_pool = 14_000_000_000 - (ledger['expected_peak_bytes'] - ledger['pool_bytes']) - 1_000_000_000
        require(report['effective_pool_slots'] == min(LAYERS * EXPERTS, available_pool // JANG_RECORD_BYTES),
                'pool size is not the checkpoint-priced explicit 14 GB plan')
    ...
    try:
        ...
        prefill_rows = [event['rows'] for event in events if event['phase'] == 'prefill' and event['layer'] == 0]
        stats = report.get('stats', {})
        require(stats.get('prefillPasses') == prefill_rows, 'scheduled prefill passes differ from observed callbacks')
        require(stats.get('prefillTokens') == len(prompt) and stats.get('promptTokens') == len(prompt),
                'scheduled prefill token count mismatch')
        require(max(prefill_rows) == chunk, 'capture did not execute the declared chunk size')
    except Exception as error:
...
def compare(directories):
    errors, measurements = [], []
    arms = [load_arm(Path(path), chunk, errors) for path, chunk in zip(directories, ARM_CHUNKS)]
    ...
    for key in sorted(expected_keys):
        row = {'field': key, 'passed': False, 'gating': not key.startswith('routes/')}
        try:
            ...
            if not row['passed'] and row['gating']:
    ...
    for phase in ('prefill', 'continuation'):
        aggregate = {'field': f'routes/{phase}/all_layers', 'gating': True, 'passed': False,
                     'metric': 'fraction_of_reference_keep_set_replaced_across_all_layers'}
        try:
            rows = [row for row in measurements if row['field'].startswith(f'routes/{phase}/')]
            require(len(rows) == LAYERS and all('control' in row and 'candidate' in row and 'error' not in row for row in rows),
                    'aggregate routing requires validated measurements for all 48 layers')
            for arm in ('control', 'candidate'):
                count = sum(row[arm]['total_rows'] * TOPK for row in rows)
                missing = sum(row[arm]['missing_experts'] for row in rows)
                require(count > 0, 'aggregate routing selection count must be positive')
                aggregate[arm] = {'missing_experts': missing, 'total_selections': count,
                                  'keep_set_disagreement': missing / count}
            aggregate['threshold'] = max(3 * aggregate['control']['keep_set_disagreement'], 0.01)
            aggregate['passed'] = aggregate['candidate']['keep_set_disagreement'] <= aggregate['threshold']
            if not aggregate['passed']:
                errors.append({'scope': aggregate['field'], 'error': 'aggregate route band or exactness gate failed'})
        except Exception as error:
            aggregate['error'] = f'{type(error).__name__}: {error}'
            errors.append({'scope': aggregate['field'], 'error': aggregate['error']})
        route_aggregates.append(aggregate)
    return {'format': 'slotstream-prefill-comparison-v1', 'schema_version': 1,
            'validator_revision': 4,
            'qualification_cohort': 'Plan020 fresh 256 reference / 384 empirical control / 512 candidate',
            'deployment_profile': 'Mac17,6 / Apple M5 Max / 25F80; fusedRoPE=false',
            'validator_correction': 'V1 used integrationCandidate fusedRoPE rather than platform deployment defaults; V2 incorrectly gated routing per layer. V3 uses the existing all-layer aggregate routing criterion per phase. State/logit thresholds and all diagnostic layer values are unchanged.',
            'passed': not errors, 'reference_chunk': 256, 'empirical_control_chunk': 384, 'candidate_chunk': 512,
            ...
            'comparison_policy': {'relative_band': 'candidate <= max(3 * control, 0.01)', 'denominator_floor': 1e-6,
                                  ...
                                  'route_gating_scope': 'all 48 layers aggregated per phase',
                                  'per_layer_route_metrics': 'diagnostic only',
                                  'route_method_source': 'Sources/SlotstreamDiagnostics/Diagnostics+PrefillQualification.swift:149-166'},
            'directories': [str(Path(x).resolve()) for x in directories],
            'errors': errors, 'measurements': measurements, 'route_aggregates': route_aggregates}
