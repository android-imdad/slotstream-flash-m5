#!/usr/bin/env python3
"""Frozen Plan020 independent 512 task screen/extension; never authorizes changing a default."""
...
FIXTURES = Path('/Users/imdad/Documents/projects/slotstream-flash-m5/.build/flash/prefill-qualification-20260916')
RUN_BASE = ROOT / '.build/flash/runs/prefill-512-qualification-20260916'
...
def fixture_binding():
    files = [FIXTURES / 'tasks.json', FIXTURES / 'task-source.json', FIXTURES / 'tasks-tokenized/prompts.json']
    corpus = read_json(files[-1])
    check(sha256(FIXTURES / 'task-source.json') == corpus['source_manifest_sha256'], 'source manifest changed')
    tasks = read_json(FIXTURES / 'tasks.json')['tasks']
    check(tuple(t['id'] for t in tasks) == TASK_IDS, 'task inventory/order differs')
    sources = {d['id']: d for d in read_json(FIXTURES / 'task-source.json')['documents']}
    expected_ids = {}
    artifacts = {}
    for task in tasks:
    ...
    for entry in corpus['documents']:
        path = (FIXTURES / 'tasks-tokenized' / entry['path']).resolve(strict=True)
        check(path.is_relative_to(FIXTURES / 'tasks-tokenized'), 'token artifact escaped fixture directory')
    ...
    hashes = {str(path.relative_to(FIXTURES)): sha256(path) for path in files}
...
def summarize(rows, extended):
    summary = {'all_quality_passed': all(r.get('quality', {}).get('passed') is True for r in rows),
               'default_admitted': False, 'historical_rows_pooled': False, 'historical_plan018_rows_pooled': False, 'historical_plan019_rows_pooled': False,
...
def validate_numeric_cohort(numeric):
    check(numeric.get('run_set_id') == 'prefill-512-qualification-20260916', 'numerical run set differs')
    for key, expected in (('reference_chunk', 256), ('empirical_control_chunk', 384), ('candidate_chunk', 512)):
        check(type(numeric.get(key)) is int and numeric[key] == expected, 'numerical chunk differs: ' + key)
...
def main():
    ...
    run_id = 'prefill-512-tasks-' + args.phase + '-20260916'
    ...
    if args.phase == 'extend':
        ...
        validate_numeric_cohort(numeric)
        ...
        screen_path = RAW_BASE / 'jang-prefill-512-tasks-screen-20260916.json'
        screen = read_json(screen_path)
        check(screen.get('completed') is True and len(screen.get('rows', [])) == 8, 'screen incomplete')
    ...
    report = {'format': 'slotstream-prefill-512-task-qualification-v1', 'run_set_id': run_id,
              ...
              'fixture_root': str(FIXTURES), 'fixture_hashes': fixtures, 'tokenized_corpus': corpus, 'tokenized_artifacts': artifacts,
    ...
    try:
        ...
        def unchanged():
            ...
            check(all(sha256(FIXTURES / p) == digest for p, digest in fixtures.items()), 'fixtures changed')
